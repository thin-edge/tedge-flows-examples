...
renew-ca
  ...
  Example:
    $0 renew-ca my-ca-private.pem my-ca-cert.pem
...
create-factory-cert
  ...
  Writes the factory certificate to <device-id>-factory-cert.b64 in the current directory.

  Example:
    $0 create-factory-cert "${DEVICE_ID:-child-001}" factory-ca-private.pem
...
enroll
  ...
  Examples:
    ...
    $0 enroll "${DEVICE_ID:-child-001}" --broker "${BROKER:-127.0.0.1}" --keygen

    # Factory cert mode, device supplies its own keypair:
    $0 create-factory-cert "${DEVICE_ID:-child-001}" factory-ca-private.pem
    $0 enroll "${DEVICE_ID:-child-001}" --broker "${BROKER:-127.0.0.1}" \\
      --factory-cert "${DEVICE_ID:-child-001}-factory-cert.b64" \\
      --factory-key "${DEVICE_ID:-child-001}-factory-device-private.pem"
...
reenroll
  ...
  Examples:
    ...
    $0 reenroll "${DEVICE_ID:-child-001}" --broker "${BROKER:-127.0.0.1}" \\
...
Full workflow:
  ...
  # 2. Per device (Phase 1, manufacturer): create factory certificate and burn onto device
  $0 create-factory-cert "${DEVICE_ID:-child-001}" factory-ca-private.pem
  # => copy ${DEVICE_ID:-child-001}-factory-cert.b64 + ${DEVICE_ID:-child-001}-factory-device-private.pem onto the device
...
# Show help if --help or -h appears anywhere in the argument list, or for the
# "help" command (e.g. `x509-cert.sh help`, `x509-cert.sh enroll --help`).
case "$COMMAND" in help|--help|-h) usage ;; esac
for _arg in "${@:-}"; do
  case "$_arg" in --help|-h) usage ;; esac
done
...
EOF
    ...
    echo "  $0 create-factory-cert <device-id> $FACTORY_CA_PRIV_PEM" >&2
    ;;
  ...
  create-factory-cert)
    ...
    # Assemble and base64-encode the factory certificate, write to file
    FACTORY_CERT_FILE="${DEVICE_ID}-factory-cert.b64"
    printf '%s' "{\"device_id\":\"${DEVICE_ID}\",\"public_key\":\"${FACTORY_DEV_PUB}\",\"_cert_sig\":\"${CERT_SIG}\"}" \
      | base64 | tr -d '\n' > "$FACTORY_CERT_FILE"
    echo "Files written:" >&2
    echo "  $FACTORY_CERT_FILE  — factory certificate" >&2
    echo "  $FACTORY_DEV_PEM  — factory device private key (keep secret)" >&2
    echo "" >&2
    echo "Copy both files to the device's secure storage:" >&2
    cat >&2 <<COPY_HINT
  scp ${FACTORY_DEV_PEM} \\
      ${DEVICE_ID}-factory-cert.b64 \\
      user@<device>:/home/user/
COPY_HINT
    ;;
